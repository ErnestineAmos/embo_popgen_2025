...
```{r}
rbinom(20, 2*N, 0.53) / (2*N)
```

```{r}
 freq <- rbinom(100, 2*N, 0.45) / (2*N)
```
...
fA[1] <- 0.50
for (t in 1:99) fA[t+1] <- rbinom(1, 2*N, fA[t]) / (2*N)
if (j == 1) plot(x=1:gen, y = fA, xlab = "generations", type = "l", ylim= c(0,1), col = blue(nrepl)[j]) else lines(x=1:gen, y = fA, col = rainbow(nrepl)[j])
}
